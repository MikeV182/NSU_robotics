        ...
        if (min_distance == std::numeric_limits<float>::max()) {
            ...
            if (!moving) {
                ...
                if (throttle_elapsed >= LOG_THROTTLE_DURATION) {
                    RCLCPP_INFO(this->get_logger(), "Moving (no obstacles)");
            ...
            cmd->linear.x = 0.0;
            if (moving) {
                // RCLCPP_WARN(this->get_logger(), "Obstacle at %.2f m - STOPPED", min_distance);
                moving = false;
            }
        } else {
            cmd->linear.x = 0.3;
            if (!moving) {
                auto throttle_elapsed = std::chrono::duration<double>(current_time - last_log_time_).count();
                if (throttle_elapsed >= LOG_THROTTLE_DURATION) {
                    RCLCPP_INFO(this->get_logger(), "Moving (clear path)");
                    last_log_time_ = current_time;
                    moving = true;
                }
            }
